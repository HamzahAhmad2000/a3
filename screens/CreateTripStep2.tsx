...
// Use improved CrossPlatformMapPicker with better UI
...
interface TripForm {
  ...
  driverType?: 'self' | 'company'; // Add driver type
...
const CreateTripStep2: React.FC = () => {
  ...
  const [selectedDriverType, setSelectedDriverType] = useState<'self' | 'company'>('self'); // Add driver selection
  ...
  const handleContinue = async () => {
    if (!selectedCar) {
      Alert.alert('Car Required', 'Please select a car type to continue.');
      return;
    }

    // Check passenger count only for self-driving
    if (selectedDriverType === 'self' && !tripData?.selectedSeats) {
      Alert.alert('Passenger Count Required', 'Please select the number of passengers.');
      return;
    }

    try {
      // Set passenger count based on driver type
      let passengerCount = tripData?.selectedSeats || 1;
      if (selectedDriverType === 'company') {
        // For company drivers, passenger count is always 1 (just the user)
        passengerCount = 1;
      }

      // Update trip data with car, driver, and passenger selections
      const updatedTripData = {
        ...tripData,
        carType: selectedCar.id,
        driverType: selectedDriverType, // Save driver selection
        selectedSeats: passengerCount, // Set the correct passenger count
        // Calculate final fare with car multiplier
        fare: Math.round((tripData.fare || 0) * selectedCar.fareMultiplier),
      };

      // Convert Date to string before saving (if it exists)
      const dataToStore = {
        ...updatedTripData,
        arrivalTime: updatedTripData.arrivalTime?.toISOString(),
      };

      // Store updated data (using 'tripForm' to match what Step3 expects)
      await AsyncStorage.setItem('tripForm', JSON.stringify(dataToStore));
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.sectionContainer}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.carScrollContainer}>
              {cars.map((car) => (
                <TouchableOpacity
                  key={car.id}
                  style={[
                    styles.carCard,
                    selectedCar?.id === car.id && styles.selectedCarCard,
                  ]}
                  onPress={() => setSelectedCar(car)}
                >
                  <Image source={car.image} style={styles.carImage} resizeMode="contain" />
                  <Text style={styles.carType}>{car.title}</Text>
                  <Text style={styles.carDescription}>{car.description}</Text>
                  <Text style={styles.carPrice}>~{Math.round(baseFareNumber * car.fareMultiplier)} Rs.</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
          ...
          {/* Driver Selection */}
          <View style={styles.sectionContainer}>
            <Text style={styles.sectionTitle}>Who's Driving?</Text>
            <View style={styles.driverSelectionContainer}>
              <TouchableOpacity
                style={[
                  styles.driverOption,
                  selectedDriverType === 'self' && styles.selectedDriverOption,
                ]}
                onPress={() => setSelectedDriverType('self')}
              >
                <View style={styles.driverOptionContent}>
                  ...
                  <View style={styles.driverInfo}>
                    <Text style={[
                      styles.driverTitle,
                      selectedDriverType === 'self' && styles.selectedDriverTitle
                    ]}>
                      I'll Drive
                    </Text>
                    <Text style={styles.driverDescription}>
                      You will drive your own vehicle
                    </Text>
                  </View>
                </View>
                <View style={[
                  styles.radioButton,
                  selectedDriverType === 'self' && styles.selectedRadioButton
                ]}>
                  {selectedDriverType === 'self' && <View style={styles.radioButtonInner} />}
                </View>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                style={[
                  styles.driverOption,
                  selectedDriverType === 'company' && styles.selectedDriverOption,
                ]}
                onPress={() => setSelectedDriverType('company')}
              >
                <View style={styles.driverOptionContent}>
                  <Image
                  ...
                  <View style={styles.driverInfo}>
                    <Text style={[
                      styles.driverTitle,
                      selectedDriverType === 'company' && styles.selectedDriverTitle
                    ]}>
                      Request Driver
                    </Text>
                    <Text style={styles.driverDescription}>
                      Our driver will pick you up
                    </Text>
                  </View>
                </View>
                <View style={[
                  styles.radioButton,
                  selectedDriverType === 'company' && styles.selectedRadioButton
                ]}>
                  {selectedDriverType === 'company' && <View style={styles.radioButtonInner} />}
                </View>
              </TouchableOpacity>
            </View>
          </View>

          {/* Passenger Selection - Only show if "I'll Drive" is selected */}
          {selectedDriverType === 'self' && (
            <View style={styles.sectionContainer}>
              <Text style={styles.sectionTitle}>Number of Passengers</Text>
              <Text style={styles.sectionSubtitle}>How many passengers can you take?</Text>
                ...
                {[1, 2, 3, 4].map((number) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      tripData?.selectedSeats === number && styles.selectedPassengerButton,
                    ]}
                    onPress={() => setTripData(prev => prev ? {...prev, selectedSeats: number} : null)}
                  >
                    <Text
                      style={[
                        styles.passengerButtonText,
                        tripData?.selectedSeats === number && styles.selectedPassengerButtonText,
                      ]}
                    >
                      {number}
                    </Text>
