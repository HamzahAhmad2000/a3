...
const Wallet: React.FC = () => {
  ...
  const loadWalletInfo = useCallback(async (showLoadingIndicator = true) => {
    ...
    try {
      ...
      lastUpdateRef.current = Date.now(); // Update timestamp
    } catch (err) {
      ...
      // Show error alert only if we don't have existing wallet data
      setWalletInfo(prevWallet => {
        if (!prevWallet) {
  ...
  // Only reload if coming from another screen (not on initial mount)
  useFocusEffect(
    useCallback(() => {
      // Skip if this is the initial load
      if (lastUpdateRef.current === 0) return;
      
      // Only reload if more than 5 seconds have passed since last update
